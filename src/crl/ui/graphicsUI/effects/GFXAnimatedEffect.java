...
public class GFXAnimatedEffect extends GFXEffect{
	...
	public void drawEffect(GFXUserInterface ui, SwingSystemInterface si){
		...
		for (int j = 0; j<frames.length; j++){
			si.drawImage(absolute.x*GFXUserInterface.TILESIZE+xoff+GFXUserInterface.UPLEFTBORDER, absolute.y*GFXUserInterface.TILESIZE+GFXUserInterface.UPLEFTBORDER-4*height+yoff, frames[j]);
			si.refresh();
			animationPause();
			si.restore();
		}
