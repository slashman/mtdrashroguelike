	...
	public void drawEffect(GFXUserInterface ui, SwingSystemInterface si){
		...
		for (int i = 0; i < depth; i++){
			...
			/*if (!ui.insideViewPort(toPrint))
				break;*/
			si.drawImage(toPrint.x()*GFXUserInterface.TILESIZE+GFXUserInterface.UPLEFTBORDER+8, toPrint.y()*GFXUserInterface.TILESIZE+GFXUserInterface.UPLEFTBORDER+8-4*height, icon);
			si.refresh();
			animationPause();
			si.restore();
		}
