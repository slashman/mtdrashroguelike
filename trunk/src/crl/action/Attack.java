	...
	public void execute(){
		...
		if (weapon == null) {
			...
			if (targetMonster != null){
				aLevel.addMessage("You punch the "+targetMonster.getDescription());
				if (Util.chance(50))
					aLevel.addMessage("but your blow is innefective!");
				else{
					targetMonster.tryHit(player, null, false, false, targetDirection);
				}
			}
			Feature targetFeature = aLevel.getFeatureAt(targetPosition);
			if (targetFeature != null && targetFeature.isDestroyable()){
				aLevel.addMessage("You punch the "+targetFeature.getDescription());
				targetFeature.damage(player, 1+player.getAttack());
         	}
			
			Cell targetMapCell = aLevel.getMapCell(targetPosition);
			if (targetMapCell != null && targetMapCell.isSolid()){
				aLevel.addMessage("You punch the "+targetMapCell.getShortDescription());
         	}			
			return;
		}
		...
		for (int i=0; i<weapon.getRange(); i++){
			...
			if (targetMonster != null){
				...
				if (targetMonster.tryHit(player, player.getWeapon(), cornered, cinetic, targetDirection)){
					if (!targetMonster.isHeavy() && player.getFlag("COMBAT_KNOCKBACK") && player.getCurrentTactic() == Player.TACTIC_BRAVE && Util.chance(75)){
						//Push the monster
...
/*		if (!hitsSomebody)
	...
	public String getSFX(){
		...
		case 0:
			return "wav/sword1.wav";
		case 1:
			return "wav/sword2.wav";
		default:
			return "wav/sword3.wav";
		}
	}
