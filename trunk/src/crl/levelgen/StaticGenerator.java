...
public class StaticGenerator {
	...
	private final String[] POTION_IDS = new String[]{"RED_POTION","YELLOW_POTION"};
    	...
    	for (int y = 0; y < map.length; y++)
			for (int x = 0; x < map[0].length(); x++) {
				...
				if (!cmds[0].equals("NOTHING"))
					try {
						cmap[0][where.x+x][where.y+y] = MapCellFactory.getMapCellFactory().getMapCell(cmds[0]);
				...
				if (cmds.length > 1){
					if (cmds[1].equals("FEATURE")){
						if (cmds.length < 4 || Util.chance(Integer.parseInt(cmds[3]))){
							...
							vFeature.setPosition(where.x+x,where.y+y,0);
							if (cmds.length > 4){
					...
					if (cmds[1].equals("ITEM")){
						...
						if (vItem != null)
							l.addItem(new Position(where.x+x,where.y+y,0), vItem);
					}else
					if (cmds[1].equals("MONSTER")){
						Monster toAdd = MonsterFactory.getFactory().buildMonster(cmds[2]);
						toAdd.setPosition(where.x+x,where.y+y,0);
						l.addMonster(toAdd);
					}else
					...
					if (cmds[1].equals("EXIT")){
						l.addExit(new Position(where.x+x,where.y+y,0), cmds[2]);
					} else
