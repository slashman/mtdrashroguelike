...
public class Monster extends Actor implements Cloneable{
	...
	public boolean tryMagicHit(Player attacker, int magicalBreak, int magicalDamage, int magicalHit, boolean showMsg, String attackDesc){
		int hitChance = 100 - getEvasion();
		...
		if (evadePoints > 0 && Util.chance(evasion)){
			...
			evadePoints -= magicalBreak + attacker.getSoulPower();
			if (evadePoints < 0)
				evadePoints = 0;
			moveRandomly();
			return false;
			...
			if (justEvaded()){
				level.addMessage("The "+attackDesc+" catches the "+getDescription()+" while evading!");
			}
			if (hasCounter("SLEEP")){
	...
	public boolean tryHit(Player attacker, Item weapon, boolean cornered, boolean cinetic, int direction){
		...
		int hitChance = 100 - getEvasion();
		if (weapon != null){
			hitChance = (int)Math.round((hitChance + weapon.getHitChance())/2.0d);
		}
		...
		int evadeBreak = 1;
		if (weapon != null)
			evadeBreak = weapon.getEvadeBreak();
		int weaponAttack = 0;
		if (weapon != null)
		...
		if (evadePoints > 0 && Util.chance(evasion)){
			attacker.doHit(false);
			if (isValidCharge)
				level.addMessage("The "+getDescription()+ " evades the charge!!");
			else
				level.addMessage("The "+getDescription()+ " dodges the attack!");
			evadePoints -= evadeBreak + attacker.getEvadeBreak() + (isValidCharge ? attacker.getEvadeBreak()  : 0);
			//level.addMessage("Break "+weapon.getEvadeBreak() + attacker.getEvadeBreak() + (attacker.isCharging() ? (int)(attacker.getEvadeBreak()/2.0f)  : 0));
			if (evadePoints < 0)
				evadePoints = 0;
			moveRandomly();
			return false;
			...
			if (justEvaded()){
				level.addMessage("You hit catches the "+getDescription()+" while evading!");
			}
			attacker.doHit(true);
			if (hasCounter("SLEEP")){
				level.addMessage("You wake up the "+getDescription()+ "!");
			...
			if (attacker.isDrashBack()){
				level.addMessage("You are in a combat frenzy!!");
			}
			int baseDamage =
				weaponAttack+
				attacker.getAttack()
	...
	public void tryLand(Position destinationPoint){
		...
		boolean destinationWalkable = aLevel.isValidCoordinate(destinationPoint) && !aLevel.isSolid(destinationPoint);
