	...
	public void drawEffect(GFXUserInterface ui, SwingSystemInterface si){
		...
		for (int i = 0; i < depth; i++){
			...
			if (!ui.insideViewPort(toPrint))
				continue;
			si.drawImage(toPrint.x()*GFXUserInterface.TILESIZE+GFXUserInterface.UPLEFTBORDER, toPrint.y()*GFXUserInterface.TILESIZE+GFXUserInterface.UPLEFTBORDER-4*height, missile[too]);
			si.refresh();
			animationPause();
			si.restore();
